...
function displayMovieRecommendation(data) {
  ...
  const recommendationContainer = movieSection.querySelector(
    ".recommendation-item"
  );
...
function displayBookRecommendation(data) {
  ...
  const recommendationContainer = bookSection.querySelector(
    ".recommendation-item"
  );
  if (recommendationContainer) {
...
function displayRecommendations() {
  try {
    const recommendationData = JSON.parse(
      localStorage.getItem("recommendationDetails")
    );
    console.log("Recommendation Data:", recommendationData);

    if (!recommendationData) {
      showError(
        "No recommendation data found. Please try the questionnaire again."
      );
      return;
    }
    ...
    if (type === "movie") {
      ...
      displayMovieRecommendation(
        recommendationData.movie || recommendationData
      );
