...
export async function loadAllRecommendations() {
  try {
    ...
    allMovieRecommendations = sortedRecs
      .filter((rec) => {
        return (
          rec.type === "movie" ||
          (rec.type === "both" &&
            rec.recommendationDetails &&
            rec.recommendationDetails.movie)
        );
      })
      .map((rec) => {
    ...
    allBookRecommendations = sortedRecs
      .filter((rec) => {
        return (
          rec.type === "book" ||
          (rec.type === "both" &&
            rec.recommendationDetails &&
            rec.recommendationDetails.book)
        );
      })
      .map((rec) => {
        return rec.type === "both"
...
function addViewAllButtons() {
  ...
  if (allMovieRecommendations.length > 1 && movieSection) {
    ...
    if (!existingButton) {
      ...
      singleRecButton.setAttribute(
        "title",
        "Get more recommendations to see your history"
      );
...
async function displayRecommendation(container, recommendation, type) {
  if (!container || !recommendation || !recommendation.recommendationDetails) {
    ...
    container.innerHTML =
      '<div class="error-message">Error: Could not display this recommendation.</div>';
    return;
  }
