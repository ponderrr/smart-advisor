import { updateEmail, updatePassword, reauthenticateWithCredential, EmailAuthProvider, signOut } from "https://www.gstatic.com/firebasejs/11.0.1/firebase-auth.js";
...
async function setupEmailForm() {
    ...
    emailForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (!user) {
                showMessage(emailForm, 'You must be logged in to change your email', true);
                return;
            }
...
function setupPasswordForm() {
    ...
    passwordForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            const user = auth.currentUser;
            if (!user) {
                showMessage(passwordForm, 'You must be logged in to change your password', true);
                return;
            }
...
function setupAgeForm() {
    ...
    ageForm.addEventListener('submit', async (e) => {
        ...
        // Validate age
        if (!newAge || isNaN(newAge) || parseInt(newAge) < 1 || parseInt(newAge) > 120) {
...
function showMessage(form, message, isError) {
    ...
    if (isError) {
        messageElement.style.backgroundColor = isDarkTheme ? 'rgba(255, 87, 87, 0.2)' : 'rgba(220, 53, 69, 0.1)';
        messageElement.style.color = isDarkTheme ? '#ff5757' : '#dc3545';
    } else {
        messageElement.style.backgroundColor = isDarkTheme ? 'rgba(75, 211, 105, 0.2)' : 'rgba(40, 167, 69, 0.1)';
    ...
    messageElement.style.border = isError 
        ? (isDarkTheme ? '1px solid rgba(255, 87, 87, 0.4)' : '1px solid rgba(220, 53, 69, 0.3)') 
        : (isDarkTheme ? '1px solid rgba(75, 211, 105, 0.4)' : '1px solid rgba(40, 167, 69, 0.3)');
