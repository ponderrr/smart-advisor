...
async function displayRecommendationInContainer(container, recommendation, type) {
    if (!container || !recommendation || !recommendation.recommendationDetails) {
        console.warn('Invalid recommendation data:', recommendation);
        container.innerHTML = '<div class="error-message">Could not display recommendation</div>';
        return;
    }
    ...
    try {
        ...
        if (posterContainer) {
            ...
            posterContainer.alt = `${recDetails.title || 'Untitled'} ${type === 'movie' ? 'Poster' : 'Cover'}`;
        }
        ...
        if (detailsContainer) {
            // Update title
            const titleElement = detailsContainer.querySelector('.recommendation-title');
            if (titleElement) {
            ...
            // Update description
            const descriptionElement = detailsContainer.querySelector('.recommendation-description');
            if (descriptionElement) {
                descriptionElement.textContent = recDetails.description || 'No description available.';
            }
        }
...
function openRecommendationsModal(type) {
    ...
    modalContent.style.background = getCurrentTheme() === 'dark'
...
function createRecommendationCarousel(type) {
    ...
    const buttonBgColor = getThemeColor('rgba(227, 66, 117, 0.3)', 'rgba(186, 38, 43, 0.3)');
...
async function navigateRecommendation(type, direction) {
    ...
    console.log(`Current ${type} index: ${currentIndex}, recommendation:`, recommendation);
    ...
    if (nextButton) {
        ...
        nextButton.style.opacity = currentIndex === recommendations.length - 1 ? '0.5' : '1';
        nextButton.style.cursor = currentIndex === recommendations.length - 1 ? 'not-allowed' : 'pointer';
    }
...
async function displayRecommendationInModal(container, recommendation, type) {
    ...
    if (!container || !recommendation || !recommendation.recommendationDetails) {
        console.warn("Invalid recommendation data:", recommendation);
        container.innerHTML = '<div class="error-message" style="background-color: rgba(255, 0, 0, 0.2); color: #ffffff; padding: 1rem; border-radius: 0.5rem; text-align: center; margin: 1rem; border: 1px solid rgba(255, 0, 0, 0.3);">Error: Could not display this recommendation.</div>';
        return;
    }
