...
import { CARD_ACCENTS } from "@/features/quiz/components/steps/content-selection-step";
import { getAccentName } from "@/features/quiz/utils/content-accent";
...
export type DemoContentType = "movie" | "book" | "music" | "mix";

export type DemoAnswerPayload = {
  id: string;
  title: string;
      ...
      <div className="grid grid-cols-[112px_1fr] sm:grid-cols-[140px_1fr] md:grid-cols-[168px_1fr]">
        ...
        <div
          className={cn(
            ...
            // Album art is square; posters/covers are 2:3.
...
interface DemoResultsContentProps {
  items: DemoItem[];
  contentType: DemoContentType;
  error: DemoResultsError | null;
  /** Restart the demo from a fresh quiz (no navigation). */
  onRetake: () => void;
  /** Send the visitor to sign-up. */
  onSignUp: () => void;
}

/**
 * Pure presentation for the demo payoff: rate-limit / generic error, or the
 * recommendation sections + sign-up CTA. No fetching, no routing, no loading
 * — the /demo orchestrator owns the phase + generation so the whole flow
 * morphs inside one card like the real quiz.
 */
export const DemoResultsContent = ({
  items,
  contentType,
  error,
  onRetake,
  onSignUp,
}: DemoResultsContentProps) => {
  const t = useTranslations("Demo.results");
  // Accent-code the sign-up buttons to the picked content type, like the
  // rest of the demo flow (amber/emerald/rose/violet).
  const accent = CARD_ACCENTS[getAccentName(contentType)];
        ...
        <div className="mt-6 flex flex-col items-center justify-center gap-3 sm:flex-row">
          {error.kind === "limit" ? (
            <PillButton
              onClick={onSignUp}
              className={cn(
                "group/su relative inline-flex w-full items-center justify-center gap-2 overflow-hidden border-transparent bg-gradient-to-r px-7 py-3 text-sm font-black tracking-tight text-white shadow-lg transition-all duration-200 hover:-translate-y-0.5 hover:scale-[1.03] hover:shadow-xl sm:w-auto",
                accent.chipGradient,
                accent.chipShadow,
              )}
            >
              {/* Sheen sweep on hover for a bit of flash. */}
              <span
                aria-hidden="true"
                className="pointer-events-none absolute inset-0 -translate-x-full bg-gradient-to-r from-transparent via-white/30 to-transparent transition-transform duration-700 group-hover/su:translate-x-full"
              />
              <Sparkles size={16} className="relative" />
              <span className="relative">{t("errors.signUp")}</span>
        ...
        <>
          ...
          {musicItems.length > 0 && (
            ...
            >
              <SectionHeader
                icon={<Music size={14} />}
                eyebrow={t("sections.music.eyebrow")}
        ...
        <div className="relative">
          ...
          <div className="mt-6 flex flex-col items-center justify-center gap-3 sm:flex-row">
            ...
            >
              ...
              />
              <Sparkles size={16} className="relative" />
              <span className="relative">{t("cta.signUp")}</span>
              <ArrowRight size={16} className="relative" />
            </PillButton>
            <PillButton
              onClick={onRetake}
