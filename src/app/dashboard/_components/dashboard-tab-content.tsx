...
import type { useDashboardData } from "../_hooks/use-dashboard-data";
import type { useAchievements } from "../_hooks/use-achievements";
import type { useMilestoneCelebrations } from "../_hooks/use-milestone-celebrations";
import type { useDashboardCopy } from "../_hooks/use-dashboard-copy";
...
type DD = ReturnType<typeof useDashboardData>;
type MC = ReturnType<typeof useMilestoneCelebrations>;
type DC = ReturnType<typeof useDashboardCopy>;
type AC = ReturnType<typeof useAchievements>;
type TabId = "overview" | "picks" | "genres" | "milestones";
type PicksFilter = "all" | "favorites";
...
interface DashboardTabContentProps
  extends Pick<
    ...
    Pick<
      ...
      | "setOpenMilestoneId"
    >,
    Pick<DC, "suggestion" | "wrappedSeason"> {
  achievements: AC;
  loading: boolean;
  ...
  setPicksFilter: (f: PicksFilter) => void;
  slideDir: number;
  setSelectedRec: (rec: Recommendation | null) => void;
  handleMarkFinished: (item: LibraryItem) => Promise<void> | void;
  renderPickCard: (rec: Recommendation) => ReactNode;
}

/**
 * The dashboard tab content (overview / picks / milestones / genres).
 * Extracted verbatim from dashboard/page.tsx — the four sequential
 * `{activeTab === "…" && (…)}` blocks moved unchanged. Owns its own
 * Dashboard / Dashboard.body i18n + router (same pattern as the other
 * extracted components); everything else is threaded in as props.
 */
