...
/* ---------- Question Skeleton ----------
 * Sits inside the same question card shell while the AI generates questions.
 * Mirrors the shape of a real question (title bar, subtitle bar, option grid)
 * so when content arrives the layout stays put and only the bars crossfade
 * into real text. A status-rotation caption keeps the user oriented. */
const QuestionSkeleton = ({
  ...
  return (
    <div className="relative" role="status" aria-live="polite">
      {/* Skeleton: title + subtitle + 4 option pills laid out exactly like a
          real question card. */}
      ...
      {/* Live status caption — dot + rotating message — anchored bottom-left
          so it sits where the page subtitle / hint would normally live. */}
...
const QuestionnairePage = () => {
  ...
  // Questionnaire is the second half of the quiz flow — content-selection +
  // question-count fill the first 50% of the bar (step 1 → 25 → 50). We map
  // the questionnaire across 50 → 100 so the bar keeps advancing instead of
  // resetting to a tiny per-question value when the page mounts.
  const progress =
    questions.length > 0
  ...
  const handlePrevious = () => {
    ...
    // No answers to lose if we're still loading questions — go straight back
    // without prompting the "are you sure" dialog.
    if (isLoading) {
  ...
  // Loading state is rendered INSIDE the shell below so the back button,
  // eyebrow, and progress bar stay mounted across question-count → loader
  // → questions. The skeleton swap is animated by AnimatePresence.
  ...
  const tone = getAccentTone(contentType);
  // While loading, the eyebrow says "Preparing your quiz" instead of a
  // numeric step that doesn't exist yet — keeps the bar's context honest.
  ...
  return (
    ...
    >
      <motion.div
        ...
        className={cn(
          "rounded-2xl border bg-gradient-to-br p-4 shadow-sm backdrop-blur-md sm:rounded-3xl sm:p-8",
          tone.surfaceBorder,
